    ...
    while (bKeepEntering)
    {
        ...
        switch (iSelection)
        {
            case 1:
                ...
                if ((fscanf(stdin, "%i", &iGradeEntry)) == 0)
                {
                    fflush(stdin);
                    goto GradeEntry;
                }
                ...
                iGradeEntry = submitAGrade(&pHead, arsName, iGradeEntry);

                // If we return status __eq__ to (-1) return %ERRORLEVEL% -1
                if (iGradeEntry == -1) { return -1; }
                else if (iGradeEntry == -2) 
                { 

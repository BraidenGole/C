...
int main(void)
{
    int multiDimensional[KMULTIROWLIMIT][KMULTICOLUMNLIMIT] = 
    {
        { 3, 4, 54, 32, 87, 1, 5, 7 },
    ...
    for (sortUntilFinished; sortUntilFinished < KTOTALMULTISIZE; ++sortUntilFinished)
    {
        for (rows = 0; rows < KMULTIROWLIMIT; ++rows)
        {
            ...
            {
                if (multiDimensional[rows][columns] > multiDimensional[rows][columns + 1])
                {
                    tempLocation = multiDimensional[rows][columns];
                    multiDimensional[rows][columns] = multiDimensional[rows][columns + 1];
                    multiDimensional[rows][columns + 1] = tempLocation;
                }
            }
        }
    }
    ...
    {   
        ...
        {
            printf("%i ", multiDimensional[sortedRows][sortedColumns]);
            ++sortedColumns;
        }
        ++sortedRows;
    }
